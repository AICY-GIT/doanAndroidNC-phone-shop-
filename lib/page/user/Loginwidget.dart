...
class _LoginWidgetState extends State<LoginWidget> {
 ...
 Future<void> _signInWithGoogle(BuildContext context) async {
  try {
    ...
    if (user != null) {
      DatabaseEvent snapshotEvent = await _userRef.child(user.uid).once();
      ...
      if (snapshot.exists) {
        // Dữ liệu đã tồn tại, không cần làm gì cả
      } else {
        // Dữ liệu chưa tồn tại, thêm dữ liệu mới vào
        await _userRef.child(user.uid).set({
          'user_id': user.uid,
          'fullName': user.displayName ?? '',
          'email': user.email ?? '',
      ...
      // Tiếp tục xử lý dựa trên dữ liệu snapshot nếu cần
    ...
    print('Đăng nhập bằng Google không thành công: $error');
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text('Đăng nhập bằng Google không thành công'),
      ),
    );
  }
}
